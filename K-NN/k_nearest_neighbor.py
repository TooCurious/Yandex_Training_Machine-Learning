...
class KNearestNeighbor:
    ...
    def compute_distances_no_loops(self, X):
        ...
        #########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        result = np.resize(np.sum(np.square(X), axis=1), (num_train, num_test)).T \
                 + np.resize(np.sum(np.square(self.X_train), axis=1), (num_test, num_train)) \
                 - 2 * np.dot(X, self.X_train.T)
        dists = dists + np.sqrt(result)
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        return dists
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            index = np.argmax(counts)  # индекс значения, которое встречается чаще всего
